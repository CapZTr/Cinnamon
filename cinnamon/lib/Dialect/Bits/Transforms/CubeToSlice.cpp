...
struct LowerMatvecIPattern : public OpRewritePattern<MatvecIOp> {
  ...
  LogicalResult matchAndRewrite(MatvecIOp op,
    ...
    Value resBitwidthVal =
        rewriter.create<arith::ConstantIntOp>(loc, bitwidth * 2, 64);
    Value mVal = rewriter.create<arith::ConstantIntOp>(loc, m, 64);

    auto resSliceType = SliceType::get(ctx, bitwidth * 2, m);
    Value resSlice =
        rewriter.create<CreateSliceOp>(loc, resSliceType, resBitwidthVal, mVal);
    ...
    auto loop = rewriter.create<scf::ForOp>(
        loc, c0Val, kIndexVal, c1Val, ValueRange{resSlice},
        [&](OpBuilder &builder, Location bodyLoc, Value iv,
          ...
          Value mulLhs = builder.create<ExtractSliceOp>(bodyLoc, layerSliceType,
                                                        cube, ivI64);
          Value mulRhs = builder.create<ExtractAndReplicateColumnOp>(
              bodyLoc, layerSliceType, slice, ivI64, mVal);
          Value product = builder.create<bits::MulIOp>(bodyLoc, resSliceType,
                                                       mulLhs, mulRhs);

          Value accSlice = builder.create<bits::AddIOp>(bodyLoc, resSliceType,
                                                        curRes, product);
...
struct LowerMatmulIPattern : public OpRewritePattern<MatmulIOp> {
  ...
  LogicalResult matchAndRewrite(MatmulIOp op,
    ...
    Value resCube = rewriter.create<CreateCubeOp>(loc, resCubeType,
                                                  resBitwidthVal, mVal, nVal);
    auto resSliceType = SliceType::get(ctx, bitwidth * 2, m);
    ...
    auto outerloop = rewriter.create<scf::ForOp>(
        loc, c0Val, layerNumVal, c1Val, ValueRange{resCube},
        [&](OpBuilder &outerBuilder, Location outerLoc, Value outerIV,
          ...
          Value layerSlice = outerBuilder.create<ExtractSliceOp>(
              outerLoc, rhsLayerSliceType, rhsCube, outerIVI64);

          Value resSlice = rewriter.create<CreateSliceOp>(loc, resSliceType,
                                                          resBitwidthVal, mVal);
          auto loop = rewriter.create<scf::ForOp>(
              outerLoc, c0Val, kIndexVal, c1Val, ValueRange{resSlice},
              [&](OpBuilder &builder, Location bodyLoc, Value iv,
                  ValueRange iterArgs) {
                Value curRes = iterArgs[0];
                Value ivI64 = builder.create<arith::IndexCastOp>(
                ...
                Value mulLhs = builder.create<ExtractSliceOp>(
                    bodyLoc, lhsLayerSliceType, lhsCube, ivI64);
                Value mulRhs = builder.create<ExtractAndReplicateColumnOp>(
                    bodyLoc, lhsLayerSliceType, layerSlice, ivI64, mVal);
                Value product = builder.create<bits::MulIOp>(
                    bodyLoc, resSliceType, mulLhs, mulRhs);

                Value accSlice = builder.create<bits::AddIOp>(
                    bodyLoc, resSliceType, curRes, product);

                builder.create<scf::YieldOp>(bodyLoc, accSlice);
              });
          Value newCube = outerBuilder.create<InsertSliceOp>(
              outerLoc, resCubeType, curCube, loop.getResult(0), outerIVI64);
...
struct LowerMulIPattern : public OpRewritePattern<MulIOp> {
  ...
  LogicalResult matchAndRewrite(MulIOp op,
    ...
    Value bitwidthVal =
        rewriter.create<arith::ConstantIntOp>(loc, operandBitWidth, 64);
    Value productBitwidthVal =
        rewriter.create<arith::ConstantIntOp>(loc, operandBitWidth * 2, 64);
    Value vecLenVal = rewriter.create<arith::ConstantIntOp>(loc, vecLen, 64);

    auto rowSliceType = SliceType::get(ctx, 1, vecLen);
    Value all1Row = rewriter.create<CreateAllOneSliceOp>(loc, rowSliceType,
                                                         cstint1, vecLenVal);

    Value c0Val = rewriter.create<arith::ConstantIndexOp>(loc, 0);
    Value c1Val = rewriter.create<arith::ConstantIndexOp>(loc, 1);
    Value bitwidthIndexVal =
        rewriter.create<arith::ConstantIndexOp>(loc, operandBitWidth);

    Value all0Slice =
        rewriter.create<CreateSliceOp>(loc, lhsType, bitwidthVal, vecLenVal);
    auto resSliceType = SliceType::get(ctx, operandBitWidth * 2, vecLen);
    Value resSlice = rewriter.create<CreateSliceOp>(
        loc, resSliceType, productBitwidthVal, vecLenVal);

    auto loop = rewriter.create<scf::ForOp>(
        loc, c0Val, bitwidthIndexVal, c1Val, ValueRange{resSlice},
        [&](OpBuilder &builder, Location bodyLoc, Value iv,
            ValueRange iterArgs) {
          Value curRes = iterArgs[0];
          Value ivI64 = builder.create<arith::IndexCastOp>(
              bodyLoc, builder.getI64Type(), iv);
          Value numToShift =
              builder.create<arith::AddIOp>(bodyLoc, ivI64, cstint1);

          Value curRow = builder.create<ExtractRowSliceOp>(
              bodyLoc, rowSliceType, rhsSlice, ivI64);
          Value maskRow =
          ...
          Value partialRes =
              builder.create<AddIOp>(bodyLoc, resSliceType, curRes, shifted);
